...
PID_class M3508_MCL_Left_Speed(20, 0, 0, 12000, 0, 0, 12000),
    M3508_MCL_Right_Speed(20, 0, 0, 12000, 0, 0, 12000);
...
f BP::stateRecovery(void)
{
    if (Recovery_Use_ContinuousPid)
    {
        M2006_BP_Con_Angle.PID_new_update(Recovery_Target_Angle, M2006_BP.mang_inf);
        M2006_BP_Con_Speed.PID_new_update(M2006_BP_Con_Angle.OUT_PID, M2006_BP.sp);
        PID_OUT = M2006_BP_Con_Speed.OUT_PID;
    }
    else
    {
        M2006_BP_Angle.PID_new_update(Recovery_Target_Angle, M2006_BP.mang_inf);
        M2006_BP_Speed.PID_new_update(M2006_BP_Angle.OUT_PID, M2006_BP.sp);
        PID_OUT = M2006_BP_Speed.OUT_PID;
    }
    return PID_OUT;
...
f BP::BP_Out_Interface(u8 YK_Mode, u8 jianshu_flag)
{
    ...
    {
        const double recovery_error = Recovery_Target_Angle - M2006_BP.mang_inf;
        const u8 crossed_target = ((Recovery_Last_Error > 0.0 && recovery_error <= 0.0) ||
                                   (Recovery_Last_Error < 0.0 && recovery_error >= 0.0));
        const u8 reached_target = fabs(recovery_error) <= BP_RECOVERY_FINISH_ERROR;
        const u8 recovery_timeout = (HAL_GetTick() - Recovery_Start_Tick) >= BP_RECOVERY_MAX_DURATION_MS;
        const u8 output_disabled = (currentMode == FsmMode::PROTECT || mcl->Mode != MCL_ON);

        if (crossed_target || reached_target || recovery_timeout || output_disabled)
        {
            Stop_Recovery(M2006_BP.mang_inf);
        }
        else
        {
            Recovery_Last_Error = recovery_error;
            One_Target_Angle = Recovery_Target_Angle;
            Continuous_Target_Angle = Recovery_Target_Angle;
            if (currentState != FsmState::RECOVERY)
                switchState(FsmState::RECOVERY);
    ...
    else if (YK_Mode == PLAYER_MODE)
    {
        ...
        else if ((((YK.shubiao.press_l) && bp->delay_2ms > 150) || (request.zimiao_status && YK.shubiao.press_l && SuperPower.mode == 2 && mcl->Mode)) && buff_mode == 0)
        {
            ...
            {
                bp->Continuous_Target_Angle += Shot_SP_1 * 22;
                bp->Prefabricate_Flag = 1;
            }
...
void BP::Start_Recovery(double current_angle, double recovery_angle, u8 use_continuous_pid)
{
    ...
    this->Recovery_Start_Tick = HAL_GetTick();
    this->One_Target_Angle = this->Recovery_Target_Angle;
    this->Continuous_Target_Angle = this->Recovery_Target_Angle;
...
void BP::BP_time_out(void)
{
    ...
    if (bp->Recovery_State == RECOVERY_ACTIVE)
    {
        bp->BP_Error = 0;
        bp->delay_2ms++;
        return;
    ...
    if ((bp->Mode != GYRO_MODE && bp->Mode != AUTO_MODE) || mcl->Mode != MCL_ON)
    {
        bp->BP_Error = 0;
        bp->delay_2ms++;
        return;
    ...
    if (!use_continuous_pid && one_pid_overload)
    {
        if (++bp->BP_Error > 200)
        {
            if (recovery_ready)
            {
                bp->Start_Recovery(M2006_BP.mang_inf, BP_RECOVERY_ONE_ANGLE, 0);
    ...
    else if (use_continuous_pid && con_pid_overload)
    {
        if (++bp->BP_Error > 200)
        {
            if (recovery_ready)
            {
                bp->Start_Recovery(M2006_BP.mang_inf, BP_RECOVERY_CON_ANGLE, 1);
                bp->Last_Blockage_Recovery_Tick = now_tick;
            }
            bp->BP_Error = 0;
        }
