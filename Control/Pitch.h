...
class PITCH
{
    public:
        f Pitch_Out;
        f Target_Angle;
        f Encoder_Target_Mang; // 编码器单圈目标角（rad）
        void set_SMCref(f Target_Angle);
        void set_Pitch_Target(f Target_Angle);
    PITCH():Pitch_Out(0),Target_Angle(0),Encoder_Target_Mang(0),Pitch_real(0),Pitch_vel(0),Pitch_acc(0),Angle_buf(0),Last_Angle(0),Encoder_Initialized(0){}
    enum class State
    {
        NORMAL,
        ENCODER,
