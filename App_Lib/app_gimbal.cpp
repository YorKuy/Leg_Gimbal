...
MOTOR_RM M6020_YAW(0x208, &CAN_2),
    M3508_MCL_Right(0x202, &CAN_1),
    M3508_MCL_Left(0x201, &CAN_1),
...
static void MODE_DEAL(void)
{
  ...
  else if(GIMBAL_088_State  != BMI088_OK && (YK_Mode == CONTROL_MODE || YK_Mode == ONLY_GIMBAL || (YK_Mode == SHOOT_MODE && !request.zimiao_status) || YK_Mode == PLAYER_MODE || YK_Mode == XTL_MODE) && !request.zimiao_status)
  {
  ...
  else if(GIMBAL_088_State != BMI088_OK && (YK_Mode == CONTROL_MODE || YK_Mode == ONLY_GIMBAL || YK_Mode == SHOOT_MODE || YK_Mode == PLAYER_MODE || YK_Mode == XTL_MODE) && !request.zimiao_status)
  {
    PITCH_Mode = ENCODER_MODE;
  }
  else if (request.zimiao_status)
  {
...
static void can2_communicate_deal(void)
{
  ...
  else
  {
    CAN_2.Send_RM(0x110,
                  ...
                  YK.DR16_rx_buffer[17] | (YK.DR16_rx_buffer[16] << 8));
  }
...
static uint8_t start_deal(void)
{
  ...
  if(GIMBAL_088_State == 0x02)
  {
    static uint8_t bmi088_error_make = 0;
    if(++bmi088_error_make < 3)
    {
      HAL_Delay(200);
      GIMBAL_088_State = GIMBAL_088.Init();
    }
